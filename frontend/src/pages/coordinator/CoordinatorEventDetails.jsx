...
const CoordinatorEventDetails = () => {
  ...
  const loadDetails = async (uid) => {
    try {
      ...
      if (ev.EventCoOrdinatorID?._id !== uid) {
        setEvent("denied");
        return;
      }
  ...
  if (event === "denied") {
    return <h4 className="text-danger p-4">Access Denied</h4>;
  }
...
export default CoordinatorEventDetails;